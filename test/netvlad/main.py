...
parser.add_argument('--cacheRefreshRate', type=int, default=100, 
...
parser.add_argument('--textPath', type=str, default="./netvlad/output_0.txt", help='Path for saving.')
parser.add_argument('--structFile', type=str, default="./0_test_data.mat", help='Path for db.')
parser.add_argument('--dataPath', type=str, default='./2/', help='Path for centroid data.')
parser.add_argument('--runsPath', type=str, default='./data_1/washington-square/0/runs/', help='Path to save runs to.')
parser.add_argument('--savePath', type=str, default='./data_1/washington-square/0/checkpoint_'+ str(time.time()), 
        help='Path to save checkpoints to in logdir. Default=checkpoints/')
parser.add_argument('--cachePath', type=str, default='./data_1/washington-square/0/', help='Path to save cache to.')
parser.add_argument('--resume', type=str, default='./2/vgg16_netvlad_checkpoint', help='Path to load checkpoint from, for resuming training or testing.')
...
parser.add_argument('--margin', type=float, default=3, help='Margin for triplet loss. Default=0.1')
parser.add_argument('--split', type=str, default='test', help='Data split to use for testing. Default is val', 
        choices=['test', 'test250k', 'train', 'val'])
parser.add_argument('--gpu', type=str, default='2', help='Data split to use for testing. Default is val')
...
def train(epoch):
    ...
    for subIter in range(subsetN):
        ...
        model.eval()
        train_set.cache = join(opt.cachePath, 'washingtonSquare' + '_feat_cache.hdf5')
...
def test(eval_set, epoch=0, write_tboard=False):
    ...
    fw = open(opt.textPath, 'a', encoding="utf-8")     
    for n in range(0,len(qFeat)):
        fw.write(eval_set.dbStruct.qImage[n].item() + ' ')
        for m in range(0,5):
            fw.write(eval_set.dbStruct.dbImage[predictions[n][m]].item() + ' ')
        fw.write('\n')
    fw.close()
...
def get_clusters(cluster_set):
    ...
    initcache = join(opt.dataPath, 'centroids', opt.arch + '_' + str(cluster_set.dataset)[2:18] + '_' + str(opt.num_clusters) + '_desc_cen.hdf5')
...
if __name__ == "__main__":
    ...
    if opt.mode.lower() == 'train':
        ...
        if opt.split.lower() == 'test':
            whole_test_set = dataset.get_whole_test_set(opt.structFile)
    ...
    if opt.mode.lower() != 'cluster':
        if opt.pooling.lower() == 'netvlad':
            ...
            if not opt.resume: 
                if opt.mode.lower() == 'train':
                    initcache = join(opt.dataPath, 'centroids', opt.arch + '_washingtonSquare_' + str(opt.num_clusters) +'_desc_cen.hdf5')
                else:

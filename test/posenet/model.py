...
class PoseLoss(nn.Module):
    ...
    def forward(self, pred_x, target_x):
        ...
        self.loss_print = [loss.item(), loss_x.item()]

        return loss, loss_x.item()
...
class ResNet(nn.Module):
    def __init__(self, base_model, fixed_weight=False, dropout_rate=0.0, bayesian = False):
        ...
        self.fc_position = nn.Linear(2048, 2, bias=True)

        init_modules = [self.fc_last, self.fc_position]
...
class ResNetSimple(nn.Module):
    def __init__(self, base_model, fixed_weight=False, dropout_rate=0.0):
        ...
        # self.fc_last = nn.Linear(feat_in, 2048, bias=True)
        self.fc_position = nn.Linear(feat_in, 2, bias=False)

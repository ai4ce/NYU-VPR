...
def img_condition(img_name):
	down = {'142f189b94131981': '20180000',' f7785e5d09d15059': '20161103', '44290495868b6534': '20161122', 'b082282c12fc5800': '20161024', '3b71b88204e3e5a9': '20161025'}
	rm = ['253069d2578fed06', '1c38d9f150354c5b', '786b0894c8ae80e6']

	for item in rm:
		if item in img_name:
			return 2		# remove
	for key in down:
		if key in img_name and int(img_name[47:55]) < int(down[key]):
			return 1		# down
	return 0				# correct
	...
	def execute(self) -> None:
		...
		if is_img:
			...
			for image in tqdm(images):
				...
				if img_condition(image_path[-1]) == 1:
					img = img.rotate(180)
				elif img_condition(image_path[-1]) == 2:
					continue

				img = img.resize((640, 480))
				img_np = np.asarray(img)
				pred = self.execute_on_img(img_np)
				pixels = img.load()
				for i in range(img.size[0]):
					for j in range(img.size[1]):
						if pred[j][i] in [125, 126, 127, 128, 175, 176, 177, 178, 180, 182]:
							pixels[i, j] = (255, 255, 255)
				save_path = '/data_1/washington-square/front/'
				if os.path.isdir(save_path):
					img.save(save_path + image_path[-1])
				else:
					print(save_path)
